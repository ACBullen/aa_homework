...
class MetaCorgiSnacks
  ...
  #   info_finder = "get_#{name}_info"
  #   tastiness_finder = "get_#{name}_tastiness"
  #    info = @snack_box.send(info_finder.to_sym, @box_id)
  #    tastiness = @snack_box.send(tastiness_finder.to_sym, @box_id)
  #    result = "Treat: #{info}: #{tastiness} "
  #    tastiness > 30 ? "*#{result}" : result
  # end
